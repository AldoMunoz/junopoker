...
public class HandService {
    ...
    private boolean isRoyalFlush(Hand hand, Card[] cards) {
        ...
        if (isFlush(hand, cards) && hand.getFiveCardHand()[4].getVal() == 10)
            return true;
        return false;
    }
    ...
    private boolean isQuads(Hand hand, Card[] cards) {
        ...
        for (int i = 1; i < cards.length; i++) {
            ...
            if (cards[i].getVal() == currVal && count == 3) {
                ...
                quads[3] = cards[i];
                // following conditions find the high card (will either be first card or next
                // card after quads
                if (cards[0].getVal() != currVal)
                    quads[4] = cards[0];
                else
                    quads[4] = cards[i + 1];
            ...
            else if (cards[i].getVal() == currVal && count < 3)
                count++;
    ...
    private boolean isFullHouse(Hand hand, Card[] cards) {
        ...
        for (int i = 0; i < cards.length; i++) {
            ...
            if (cards[i].getVal() == tripVal)
                i += 2;
            // finds the first non-trip value card
            else if (pairVal == 0) {
                pairVal = cards[i].getVal();
            }
            // if a pair is found, they are made the 4th and 5th cards of the hand and
            // returned true
            ...
            // if the pair card does not match the current card, the pair card is set to the
            // current card
            else {
                pairVal = cards[i].getVal();
            }
    ...
    private boolean isStraight(Hand hand, Card[] cards) {
        ...
        for (int i = 1; i < cards.length; i++) {
            ...
            if (cards[i].getVal() == cards[i - 1].getVal())
                continue;
            ...
            // if thd current card is not one less than the previous, the array is
            // reinitialized and count is set back to 1
            else {
                straight = new Card[5];
                straight[0] = cards[i];
                count = 1;
    ...
    private boolean isThreeOfAKind(Hand hand, Card[] cards) {
        ...
        for (int i = 1; i < cards.length; i++) {
            // when found, the three of a kind is added to the first 3 positions of final
            // hand
            if (cards[i].getVal() == tripVal && count == 2) {
                ...
                // the last two positions, which will be high cards, are determined by iterating
                // the list again
                int pos = 3;
                for (int j = 0; pos < 5; j++) {
                    // when we encounter the trip value, we skip them by adding 2+1 to j
                    if (cards[j].getVal() == tripVal)
                        j += 2;
                    // if the current value isn't the trip value, it gets added to the five card
                    // hand
                    else {
            ...
            // this finds a pair
            else if (cards[i].getVal() == tripVal && count == 1)
                count++;
